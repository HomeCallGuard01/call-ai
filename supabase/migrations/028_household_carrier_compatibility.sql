...
-- evaluateHouseholdCheckoutEligibility. providerPolicy.js itself (ported
-- from branch p0-batch1-carrier-policy-quarantine, unmodified) already
-- built the compatibility POLICY (which carriers are safe), but had no
-- way to know which carrier a given household is actually on — the
-- carrier-compatibility audit found evaluateProviderCompatibility had
-- zero callers anywhere in the app as a direct result. This migration
-- adds only the storage; the gating logic itself stays entirely in
...
-- expected to grow over time, and an unrecognised key here is already
-- handled safely by getProviderPolicy's fallback to PROVIDER_POLICY.other
-- ('unverified', blocked) rather than a database error.
--
-- carrier_compatibility_captured_at is set every time this RPC runs, not
-- idempotent-once — a customer can change their selection (corrected a
-- mistake, actually switched carrier), and the most recent capture is
-- what matters, not the first.
...
begin
  if not exists (
    ...
    raise exception 'MIGRATION 028 VERIFICATION FAILED: households.carrier_provider_key missing or wrong type';
  end if;
  ...
  if not exists (
    ...
    raise exception 'MIGRATION 028 VERIFICATION FAILED: households.carrier_tariff_type missing or wrong type';
  end if;
  ...
  if not exists (
    ...
    raise exception 'MIGRATION 028 VERIFICATION FAILED: households.carrier_compatibility_captured_at missing or wrong type';
  end if;
  ...
  if not exists (
    ...
    raise exception 'MIGRATION 028 VERIFICATION FAILED: set_household_carrier_compatibility function missing';
  end if;
end
$$;
