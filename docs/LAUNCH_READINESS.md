...
- Automated tests: `tests/checkout-existing-subscription.test.mjs` (active/trialing/past_due block creation, no qualifying subscription allows it), `tests/subscribe-button.test.mjs` (button disables and updates text on submission, stays disabled on repeat invocation)

**Follow-up (18 July 2026): abandoned/still-open checkout gap closed.** A concurrency investigation found the fix above still had a gap — a Checkout Session that's created but never paid has no Subscription object at all, so the `active`/`trialing`/`past_due` check couldn't see it. A household that opened Checkout, didn't finish (closed the tab, got distracted), then tried again later would still get a second, independent Checkout Session. Closed by also querying `stripe.checkout.sessions.list({customer, status: "open"})` before creating a new session: if a still-open one exists, the customer is redirected back to it (its own `url`) instead of a new one being created; if Stripe doesn't return a usable `url`, redirected to the dashboard with a `checkout=pending` banner instead. Completed/expired sessions are explicitly excluded and never block a fresh attempt. No new schema — purely an additional Stripe API check, same shape as the existing subscription check. Tests: `tests/checkout-existing-subscription.test.mjs` (open session reused, no open session allows creation, complete/expired sessions don't block, existing subscription checks unchanged).
...
An audit of `main` found the app was not deployable as-is. Fixed:

- `package.json`: `"startt"` (typo, never actually wired up) renamed to `"start": "node server.js"` — Railway's Nixpacks build defaults to running `npm start`, so without this the deploy would never launch. Added `"engines": { "node": ">=20" }` so Nixpacks picks a consistent Node version.
- `server.js`: hardcoded `app.listen(3000, ...)` replaced with `resolvePort(process.env)` (`services/serverConfig.js`), reading `process.env.PORT` with a `3000` fallback for local dev — Railway assigns a dynamic port via `PORT` and routes traffic to it, so the hardcoded port meant the deployed app would never actually be reachable.
- Removed two leftover debug routes: `/test-db` (no authentication at all — leaked `SUPABASE_URL` to any anonymous request) and `/test-get-contacts` (had auth, but confirmed unused by any automated test or current tooling — only referenced in historical Sprint 9 docs as a manual verification aid).
- Added `.env.example` — every env var the app reads is now documented with a placeholder value, including an explicit note that no Twilio variables are currently read by any code (the `twilio` package is only used for its TwiML `VoiceResponse` builder, not the REST API client) and that `Resend_API_Key` isn't currently consumed either (email sending goes through Supabase Auth's built-in email, not Resend directly).
- Added production startup validation (`services/serverConfig.js`'s `validateProductionEnv`): when `NODE_ENV=production`, the app now refuses to start — with a clear list of *variable names* only, never values — if `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `APP_URL`, `STRIPE_SECRET_KEY`, `STRIPE_PRICE_ID`, or `STRIPE_WEBHOOK_SECRET` are missing, **or if `APP_URL` still resolves to `localhost`/`127.0.0.1`** even though it's set. Verified by actually running `NODE_ENV=production node server.js` against the current dev `.env` (which still has `APP_URL=http://localhost:3000`) — it correctly refused to boot. Local development is unaffected (this check only runs when `NODE_ENV=production`).
- Railway: no `railway.json` added — the fixes above (`start` script, `engines`, `PORT` handling) are exactly what Nixpacks' standard Node auto-detection needs; a Railway-specific config file wasn't judged to add real benefit over relying on Nixpacks. No Dockerfile added either.
- Tests added: `tests/server-config.test.mjs` (`resolvePort` PORT/fallback behavior, `validateProductionEnv` for every required var missing individually, the explicit localhost/127.0.0.1 rejection, malformed-URL handling, and that no problem message ever contains an actual env var value).

**Still required before an actual deploy (not done here, and explicitly not in scope for this pass):** setting the real production env vars in Railway itself — including `NODE_ENV=production`, the real `APP_URL`, and live (not test-mode) Stripe keys — and actually triggering a deploy. No Railway environment variables were changed and no deploy was made.
