...
- **Duplicate checkout sessions:** a single household ended up with two completed Checkout Sessions/subscriptions during testing. The idempotency-key window in `routes/billing.js` (`POST /billing/create-checkout-session`) should be reviewed before launch to understand how this happened and whether the current 5-minute bucket is sufficient.
