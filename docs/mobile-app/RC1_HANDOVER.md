...
**Update, 2026-07-30 — staging environment created, migration 021 applied and verified there.** `docs/engineering/STAGING_ENVIRONMENT_PLAN.md` §1–2 executed: staging project `home-call-guard-staging` (ref `tigwgmayeuisrxjjykqd`) created, migrations `000` (a newly-drafted baseline for `public.contacts`, which turned out to have no `CREATE TABLE` anywhere in this repo either — see `docs/engineering/MIGRATION_RECOVERY_PLAN.md`) through `021` applied via `supabase db push`, with `005_household_rls.sql` correctly excluded (frozen, would have collided with 008 — full reasoning in the recovery plan). Verified directly against staging: `households.activation_verified_at` exists, `mark_household_activation_verified` exists with the exact source-file-matching `SECURITY DEFINER` body.

**However, verification also found the execute grant is not what the migration intended.** `mark_household_activation_verified` has `EXECUTE` granted to `anon` and `authenticated` on staging, not just `service_role` — and so does every other `SECURITY DEFINER` RPC in this codebase, a Supabase platform-default-privileges issue, not something specific to 021's own SQL. See `docs/engineering/MIGRATION_RECOVERY_PLAN.md`'s Execution Outcome for full detail. **Migration 021 is still not applied to production** — only to staging — and this newly-found grant issue should be resolved (a new forward migration, not a hand-edit) and re-verified before it is.
...
1. **Migration 021 is applied and verified on staging, but not yet on production, and a newly-found grant issue should be fixed first.** The staging environment now exists (`docs/engineering/STAGING_ENVIRONMENT_PLAN.md` §1–2 executed 2026-07-30) and migration 021 applies cleanly there — B5's real success path is no longer blocked by "nowhere safe to test it." But applying the full migration set to a real (non-PGlite) database surfaced that `mark_household_activation_verified`, along with every other `SECURITY DEFINER` RPC in this codebase, unintentionally grants `EXECUTE` to `anon`/`authenticated` as well as `service_role` (Supabase platform default-privileges behavior, not a 021-specific bug — see `docs/engineering/MIGRATION_RECOVERY_PLAN.md`). Recommend fixing this with a new forward migration and re-verifying on staging before promoting 021 to production, rather than promoting now and fixing the grant issue separately afterward.
