...
**Status:** §1 and §2 executed, 2026-07-30; §5's standing verification script now built and in use as of 2026-07-31. Staging project `home-call-guard-staging` (ref `tigwgmayeuisrxjjykqd`, `eu-west-2`, free tier) created and linked; migrations `000, 001–004, 006–022` applied via `supabase db push`, verified schema-correct — `022_lock_down_security_definer_execute_grants.sql` (the fix for the gap this section originally flagged) is now applied and verified on staging too. `scripts/verify-security-definer-grants.js` is the standing artifact §5 called for — dynamic (discovers every `SECURITY DEFINER` function from `pg_proc`, not a hardcoded list), reusable unchanged against any linked project. §3 (env vars), §4 (fixtures), §6 (promotion to production — migrations 021/022 are deliberately not yet promoted, a separate controlled change), §7 (anti-cross-contamination assertion), and §8 (backup/rollback procedure) remain **not yet done**.
...
**Remaining steps below still require separate, explicit approval before execution.**

**Note added 2026-07-30, resolved 2026-07-31, relevant to §5 and §6:** applying the full migration set to staging surfaced that every `SECURITY DEFINER` RPC function in this codebase (not just 021) had `EXECUTE` granted to `anon`/`authenticated` in addition to `service_role`, despite each migration's own `revoke all ... from public` — because Supabase's default privileges grant execute directly to those roles, not via `PUBLIC`, so the revoke never reached them. This was exactly the failure mode §5 was designed to catch ("the command exited 0 is not sufficient evidence on its own"). Fixed on staging via migration 022 and verified via the now-built `scripts/verify-security-definer-grants.js`; production was separately confirmed (read-only) to never have had this exposure, and remains untouched. Migrations 021 and 022 are deliberately not yet promoted to production — see §6, still not done — full detail in `docs/engineering/MIGRATION_RECOVERY_PLAN.md`'s "Follow-up: migration 022" section.
