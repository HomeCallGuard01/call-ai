...
**Status:** §1 and §2 executed, 2026-07-30. Staging project `home-call-guard-staging` (ref `tigwgmayeuisrxjjykqd`, `eu-west-2`, free tier) created and linked; migrations `000, 001–004, 006–021` applied via `supabase db push`, verified schema-correct. §3 (env vars), §4 (fixtures), §5 (the standing verification script — ad hoc equivalents were run instead, see `MIGRATION_RECOVERY_PLAN.md`), §6 (promotion to production), §7 (anti-cross-contamination assertion), and §8 (backup/rollback procedure) are **not yet done**. Production credit: this exercise directly surfaced a real, previously-undetected gap — see the note at the end of this section — vindicating §5's original design intent even though the script itself wasn't built yet.
...
**Note added 2026-07-30, relevant to §5 and §6:** applying the full migration set to staging surfaced that every `SECURITY DEFINER` RPC function in this codebase (not just 021) has `EXECUTE` granted to `anon`/`authenticated` in addition to `service_role`, despite each migration's own `revoke all ... from public` — because Supabase's default privileges grant execute directly to those roles, not via `PUBLIC`, so the revoke never reaches them. Full detail in `docs/engineering/MIGRATION_RECOVERY_PLAN.md`'s "Execution Outcome" section. This is exactly the failure mode §5 was designed to catch ("the command exited 0 is not sufficient evidence on its own") — reinforces that §5's script must be built as a real, standing artifact before any promotion to production happens, not treated as optional.
