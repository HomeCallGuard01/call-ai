...
- [x] **Real signup exercised live against staging, 2026-08-02 through 2026-08-05, fully completed.** Registered through the actual `/register` screen. Confirmed: `POST /auth/v1/signup` returns 200; the new Supabase Auth user has `confirmation_sent_at` set but no `email_confirmed_at` until the real link is clicked — **email verification is genuinely enforced**, not bypassable from the client. Yahoo `+`-tagged alias delivery failed (Yahoo provider limitation, not a bug); a real distinct Gmail address delivered correctly, the real confirmation link was clicked, and `email_confirmed_at` was confirmed set server-side.
- [x] **Defect found and fixed: logout was completely non-functional on web.** `handleLogout()` used `Alert.alert`'s multi-button confirmation, which is a silent no-op on React Native Web — no dialog, no network call, callback never fires. Fixed with a `Platform.OS === "web"` branch to `window.confirm`, native behaviour untouched. Commit `6e17659`.
- [x] **Second defect found and fixed, 2026-08-05: signing out never navigated anywhere.** `AuthContext`'s own header comment always claimed a null session triggers a redirect to login "from wherever the customer happens to be" (E4), but the `onAuthStateChange` callback never actually called `router.replace`. Logging out from a screen that fetches dashboard data (e.g. Account) correctly cleared the session but left that screen showing an infinite loading spinner forever. Fixed by adding the redirect on the `SIGNED_OUT` event. Commit `6407010`. Verified live: logout now correctly lands on the Welcome carousel; a fresh login immediately restores full state (membership, protection, contact).
- [x] **Full login exercised live against staging, 2026-08-05** — password login establishes a real session, dashboard/bootstrap fetch sequence confirmed correct.
- [ ] Live sign-up / login / password-reset flow exercised on a real device — requires mobile-device testing
- [x] Session refresh/expiry behaviour (return to exact prior screen after re-auth) — the `SIGNED_OUT` half now live-verified per above; the "return to exactly where they were" re-auth behaviour itself still not live-tested this pass (requires deliberately expiring a session mid-flow)
...
- [x] **Live Stripe Checkout end-to-end exercised against staging, 2026-08-05** — real Stripe Sandbox checkout completed with `4242 4242 4242 4242`; `stripe_customer_id` correctly attached to the household synchronously at session-creation time. Along the way, found and fixed a real staging-environment defect (not an app-code bug): the backend was silently drawing its Stripe/Twilio/OpenAI config from the sandbox repo's default `.env` (which also carries a live-prefixed key) instead of any file actually named "staging," because `server.js` always additionally loaded cwd's `.env` regardless of what was explicitly passed at launch — this meant no `STRIPE_WEBHOOK_SECRET` was ever correctly wired up, so the subscription/entitlement webhook silently never arrived. Fixed: `server.js` now loads exactly one explicit `ENV_FILE` with no fallback merge, plus new boot-time safeguards refusing to start against a production Supabase ref or a live Stripe key (`services/serverConfig.js`, commit `24a07cb`). After the fix, the real missed webhook event was replayed and correctly created exactly one `subscriptions` row and one `entitlements` row — confirmed idempotent by replaying it twice with no duplicates.
- [ ] 30-day money-back guarantee / refund path — not tested
- [ ] Legal review of guarantee and "start immediately" consent wording — not performed; still flagged directly in `mobile/app/(setup)/subscribe.tsx`'s own code comment; see **Deferred items**
...
- [x] **Manual contact add exercised live against staging, 2026-08-05** — added via the real UI, confirmed persisted server-side and correctly household-scoped (exactly one `contacts` row, matching the one test household). Native "Choose from Contacts" picker correctly fails open with a clear message on web (`expo-contacts` isn't available in browser preview — expected, not a bug).
...
- [x] **Defect found and fixed, 2026-08-05: `not_provisioned` (409, household's Twilio number not yet assigned) caused a silent infinite redirect loop.** `activate.tsx` routed to `/(setup)/welcome` on this response, but `welcome.tsx`'s `resumeSetupAt()` has no concept of "provisioning in progress" and, since activation isn't verified yet either, always sent the customer straight back to `device-picker` — a zero-explanation bounce on tapping any device. Confirmed this is a real, customer-reachable state on production too (not just staging's blank Twilio credentials) — see the UK Twilio Address blocker in `docs/mobile-app/RC1_HANDOVER.md`. Fixed: shows an inline "Still setting up your line" state with its own retry, on the same screen, instead of looping through a target that was never built to handle this case. Commit `6407010`.
- [x] **Activation-verification flow re-exercised end-to-end against staging, 2026-08-05**, this time carried through to a real completed setup (device/provider selection → activation instructions → synthetic-call-fixture verification → "You're protected" → Home dashboard showing Protected). A synthetic Twilio number was assigned via the existing `assign_household_twilio_number` RPC (a UK Ofcom-reserved fictional number, never real) purely to get past the `not_provisioned` state for this test.
...
- [x] **Full final dashboard/account state confirmed live against staging, 2026-08-05** — Home shows "Protected"; Account tab shows Membership: Active, Protection: Protected, correct email. Confirmed persists correctly through logout→login and browser refresh (Home and Account tabs both tested).
- [ ] Account / billing-portal live flow — not tested this pass

## Emails

- [x] **Signup confirmation email exercised live, 2026-08-02 through 2026-08-05** — genuinely sent by Supabase, genuinely enforced client-side, real delivery confirmed to a real Gmail address after a Yahoo `+`-alias delivery failure (provider limitation). Password-reset and receipt emails not exercised this pass.

## Company information

- [x] **Full company-identity audit performed, 2026-08-05**, across both this repo and `/Users/ad/call-ai` (`public/`, `mobile/app/`): support email (`support@homecallguard.co.uk`), legal entity naming ("AFMD Ltd" used correctly, never asserted incorrectly), all `mailto:` links, page footers, and Stripe checkout copy were all already correct — no changes needed there. One real finding: the Privacy Policy and Terms pages had a live `[REGISTERED OFFICE ADDRESS TO BE CONFIRMED]` / `[BUSINESS POSTAL ADDRESS TO BE CONFIRMED]` placeholder (already flagged as a known launch blocker in `docs/launch/KNOWN_ISSUES.md`), filled in with the official registered office (128 City Road, London, EC1V 2NX, United Kingdom) in both `public/privacy.html` and `public/terms.html`. Commit `4524c9f`. The identical fix was also applied to `/Users/ad/call-ai`'s copies of the same two files but left uncommitted there pending owner review, since that repo already had unrelated pending changes.
- [ ] A separate, unrelated data-retention-periods placeholder in `privacy.html` — not addressed, requires an actual business-policy decision
- [ ] Stripe Dashboard's own statement descriptor / public business details — outside this repo, not auditable from code; verify separately
