...
function buildVoiceAccessToken({
  ...
  // actually be invoked.
  // pushCredentialSid is optional: without it, registration still
  // succeeds (the JWT/identity handshake doesn't require it), but Twilio
  // has no way to attach an FCM Push Credential to the resulting client
  // binding, so incoming calls fail with error 52004 ("Credential Sid is
  // null") — silent from the JS side (registerForIncomingCalls() still
  // resolves), only visible via the call's own Twilio notifications.
