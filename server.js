...
function dialHouseholdOrFailClosed(twiml, household) {
  ...
  if (plan.mode === "client-only") {
    // ringTone: "uk" (2026-09-13 fix, bf3abe4 — brought into this release
    // path 2026-09-24) — confirmed via the installed Twilio SDK's own
    // type definitions (node_modules/twilio/lib/twiml/VoiceResponse.d.ts):
    // "Ringtone allows you to override the ringback tone that Twilio will
    // play back to the caller while executing the Dial." A <Dial><Client>
    ...
    // behaviour, monitoring (attachLiveMonitoring attaches <Start><Stream>
    // to twiml directly, a sibling verb, never nested in this dial), or
    // anything decideCallDeliveryPlan decides. Confirmed NOT deployed to
